...
```
docker run -it --runtime=habana --ipc=host --rm --name unet-hpu pytorch-unet-hpu
```

**Run Training on 8 HPU**
```
docker run -it --runtime=habana --ipc=host --rm --name unet-hpu pytorch-unet-hpu python3 main.py --hpus 8
```

**Run Training on CPU**
```
docker run -it --ipc=host --rm --name unet-cpu pytorch-unet-hpu python3 main.py --cpus 1 --device cpu --framework pytorch-lightning
```
