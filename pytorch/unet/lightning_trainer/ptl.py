...
def ptlrun(args):
    ...
    trainer = Trainer(
        ...
        devices=args.hpus if args.hpus else args.gpus if args.gpus else args.cpus,
        accelerator=HPUAccelerator() if args.hpus else "gpu" if args.gpus else "cpu",
        benchmark=True,
        deterministic=False,
        ...
        strategy=HPUDDPStrategy(parallel_devices=parallel_hpus, bucket_cap_mb=args.bucket_cap_mb,gradient_as_bucket_view=True,static_graph=True) if args.hpus > 1 else SingleHPUStrategy() if args.hpus == 1 else "auto",
